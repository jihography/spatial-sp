---
title: |
  | [학생자율연구: 심화]{style="font-size:0.6em"}
...
- Motivating한 예시: 운동(X)과 피부암 발생률(Y), 태양 노출 빈도(Z)의 관계
  - 전역적 모형이 어떤 process를 모델링 하는지 명확하지 않음
...
- 관측되지 않은 **교란변수 Z**를 DGP에 명시적으로 도입하여 시뮬레이션 진행 (X와의 관련성 & **Z의 공간적 군집 정도**) → SP 발생하는지 확인
...
|---------|---------|---------|----------|
| 높음 | 높음 | — | **GWR이 역설 완화**^[역설 완화: 추정된 $\hat{\beta}$가 true $\beta$에 더 가까워짐을 의미] |
...
::: {.incremental}
- 기여: 국지적 모형(GWR, SGWR)의 장점/해석 시 유의점? 
- '인과' 프레임워크와의 구분 필요
- 실제 분석 상황: DGP를 모르는 상태
- 적절한 실증 분석 데이터셋 찾기 (예: 교란변수 Z가 공간적으로 군집되지 않은 경우)
